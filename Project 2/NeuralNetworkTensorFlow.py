...
class NeuralNetworkTensorflow:
    ...
    def create_DNN(self):
        with tf.compat.v1.name_scope('DNN'):   #In summary, tf.name_scope just add a prefix to all tensor created in that scope (except the vars created with tf.get_variable), and tf.variable_scope add a prefix to the variables created with tf.get_variable.
    ...
    def create_optimiser(self):
        with tf.compat.v1.name_scope('optimizer'):
            self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=self.eta).minimize(self.loss,
